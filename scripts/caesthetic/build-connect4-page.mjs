...
${section('implementation-options','After the audit','Turn the finding into a defined piece of work.',`<div class="c4-prose"><p>See <a href="/connect4/what-we-build/">what we build across the four surfaces</a>: a website catalog, social conversation, Maps packaging with an honest review loop, and relevant local platforms—using one demand-language set.</p><p>When location details conflict, we <a href="/connect4/location-alignment/">align information across Maps, your website, social media and reputation platforms</a>. When feedback collection needs a dependable process, we implement <a href="/connect4/review-system/">our review and feedback system</a>.</p><p>The audit identifies the need. The written scope states what we will change and how you will check it.</p><p>Explore the problem first: <a href="/blog/consistent-business-information/">why business information should agree</a> or <a href="/blog/customer-reviews-and-feedback/">how reviews and feedback help you improve</a>.</p></div>`)}
...
export function build(check=false) {
  ...
  let footer=oldFooter.includes('href="/connect4/"')?oldFooter:oldFooter.replace('<a href="/growth-score/">Free Growth Score</a>','<a href="/connect4/">How Connect4 works</a>\n        <a href="/growth-score/">Free Growth Score</a>');
  if(!footer.includes('href="/connect4/what-we-build/"')) footer=footer.replace('<a href="/connect4/">How Connect4 works</a>','<a href="/connect4/">How Connect4 works</a>\n        <a href="/connect4/what-we-build/">What we build for Connect4</a>');
  ...
  for(const image of IMAGES){
    ...
    registry.entries[image.id]={media_id:image.id,src:image.src,type:'image',kind:'approved-conceptual-illustration',state:'approved',purpose:`Owner-provided ${image.role} conceptual illustration; approved for Connect4 by the 2026-09-05 instruction, not client evidence.`,alt:`Connect4 ${image.role} illustration in ${image.format} format.`,width:image.width,height:image.height,sha256:image.sha256,format:image.format,source:image.source,rights:{status:'approved',source:SSOT+'#connect4-engagement-path',approval:image.approval||'Owner instruction 2026-09-05T14:18:08Z'},consent:{required:false,status:'not_required'},allowed_channels:['public','local','protected-preview'],allowed_routes:['/connect4/','/connect4/what-we-build/'],transformations:'none; original bytes locked'};
  }
  ...
  if(fs.existsSync(path.join(ROOT,sitemapPath))){
    ...
    if(!xml.includes('https://caesthetic.com/connect4/')) xml=xml.replace('</urlset>','  <url><loc>https://caesthetic.com/connect4/</loc></url>\n</urlset>');
    if(!xml.includes('https://caesthetic.com/connect4/what-we-build/')) xml=xml.replace('  <url><loc>https://caesthetic.com/connect4/</loc></url>','  <url><loc>https://caesthetic.com/connect4/</loc></url>\n  <url><loc>https://caesthetic.com/connect4/what-we-build/</loc></url>');
    outputs.set(sitemapPath,xml);
  }
