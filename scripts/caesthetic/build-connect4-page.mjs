...
export const IMAGES = [
  { id:'connect4.journey.desktop', source:'docs/ssot/assets/caesthetic/connect4-patient-journey-v1.png', src:'/assets/connect4/connect4-patient-journey-v1.png', width:1536, height:1024, sha256:'229821d3ef70194dedd278fff76841e1cea671148e8e06807177d9f26781b180', format:'desktop' },
  { id:'connect4.journey.mobile', source:'docs/ssot/assets/caesthetic/connect4-patient-journey-mobile-v1.png', src:'/assets/connect4/connect4-patient-journey-mobile-v1.png', width:841, height:1870, sha256:'68ec264d36113b954c7b770bb956d687ce35a7b8615a2fc28f6fcfff09f43a56', format:'mobile' }
];
...
export function getCopy(md) {
  ...
  return { contract:CONTRACT, source:SSOT, source_sha256:hash(md),
...
export function render(copy, header, footer) {
  ...
  const faq = [
    ...
    ['Is ongoing management required?','No. You keep the completed materials and operating instructions within the agreed scope. Your team, your marketer, or CAESTHETIC can maintain the system. Third-party platforms and licenses remain subject to their own terms.']
  ];
  const body = `
...
<nav class="c4-toc" aria-label="On this page"><a href="#four-surfaces">The four surfaces</a><a href="#service-example">One service</a><a href="#implementation">The work</a><a href="#lead-intake">After an inquiry</a><a href="#handoff">What you keep</a></nav>
</div></section>
...
${section('four-surfaces','02 / Connect4','Four surfaces. One clear offer.',`<dl class="c4-surfaces">${surfaces.map(([name,role,desc],i)=>`<div data-surface="${['search','website','social','reputation'][i]}"><dt><span class="c4-surface-number">0${i+1}</span>${esc(name)}</dt><dd><strong>${esc(role)}</strong><p>${esc(desc)}</p></dd></div>`).join('')}</dl><div class="c4-owner"><h3>One accountable lead.</h3><p>Different specialists can do the work. One named lead coordinates the shared facts, priorities, and updates across all four. Consistency means the same reality—not identical text or simultaneous posts.</p></div><figure class="c4-figure"><picture data-connect4-picture="journey" data-media-id="connect4.journey.desktop"><source media="(max-width: 767px)" srcset="${mobile.src}" width="${mobile.width}" height="${mobile.height}"><img src="${desktop.src}" width="${desktop.width}" height="${desktop.height}" alt="An illustrative aesthetic-practice journey: a potential client checks Search and Maps, the Website and Reviews and Reputation, then makes an inquiry. Social remains part of the connected system. Lead Intake continues with a response, follow-up and a possible booked consultation." loading="lazy" decoding="async" aria-describedby="journey-caption"></picture><figcaption id="journey-caption"><strong>One possible route. Not a required sequence.</strong> This aesthetic-practice example uses patient terminology. People can start on different surfaces and contact the business in different ways. The image illustrates a journey, not a measured client result or the assignment of responsibility.</figcaption></figure><details class="c4-details"><summary>Read the image explanation</summary><div><p>Four public surfaces surround the business. The dashed connection means they belong to one system. The burgundy arrows show one possible route through Search &amp; Maps, Website, and Reviews &amp; Reputation; Social remains connected even when this person does not visit it.</p><p>After the decision to contact, an inquiry enters Lead Intake: response, follow-up, and an appropriate next step. A booked consultation is one possible outcome, not a promise. Conflicting information or an unclear response can introduce friction; the illustration does not establish that either is happening in your business.</p></div></details>`,'c4-soft')}
...
].map(([name,before,after])=>`<article role="listitem"><h3>${esc(name)}</h3><div><p class="c4-label">The gap</p><p>${esc(before)}</p></div><div><p class="c4-label">The agreed change</p><p>${esc(after)}</p></div></article>`).join('')}</div><p class="c4-prose c4-after">The result we can check here is a coherent explanation and working next step. More bookings would require separate, comparable evidence.</p>`)}
...
export function build(check=false) {
  ...
  for(const image of IMAGES){
    ...
    registry.entries[image.id]={media_id:image.id,src:image.src,type:'image',kind:'approved-conceptual-illustration',state:'approved',purpose:'Connect4 journey; explicitly labeled aesthetic-practice example, not a general ownership diagram.',alt:'One possible client journey through the four connected public surfaces and separate Lead Intake.',width:image.width,height:image.height,sha256:image.sha256,format:image.format,source:image.source,rights:{status:'owner-provided',source:SSOT+'#connect4-visuals'},consent:{required:false,status:'not_required'},allowed_channels:['public','local','protected-preview'],allowed_routes:['/connect4/'],transformations:'none; original bytes locked'};
  }
  ...
  console.log(JSON.stringify({status:'PASS',mode:check?'check':'build',contract:CONTRACT,output_files:[...outputs.keys()],published_image_pairs:1,remaining_image_pairs:['system','service-example','delivery-handoff','paid-traffic-readiness']},null,2));
}
