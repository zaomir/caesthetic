...
const originalImages=manifest.files.map(file=>({...file,source:'site-caesthetic'+file.src,role:roleFor(file.name),format:file.name.includes('portrait')?'mobile':file.name.includes('recolored')?'alternate':'desktop',id:`connect4.owner.${roleFor(file.name)}.${file.name.includes('portrait')?'mobile':file.name.includes('recolored')?'alternate':'desktop'}`}));
export const ENGAGEMENT_IMAGES=JSON.parse(fs.readFileSync(path.join(root,'site-caesthetic/assets/connect4/engagement-20260905/manifest.json'),'utf8')).files;
export const OWNER_IMAGES=[...originalImages,...ENGAGEMENT_IMAGES];
const esc=s=>String(s).replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;').replaceAll('"','&quot;');
const text={
 engagement:{alt:'Five engagement options: Free Growth Score, optional Lead-to-Revenue Check, 30-Day Growth Sprint, optional additional Sprints, and optional Ongoing Growth System. Each continuation is a separate decision.',caption:'You choose what you need. The Check, additional Sprints and ongoing Growth System are optional. No automatic renewal. Scope and terms are explained below.'},
...
export function engagementMarkup(copy){
 const c=copy.en;
 return `<section id="working-together" class="c4-section c4-soft" data-engagement-contract="${copy.contract}" aria-labelledby="working-together-title" tabindex="-1"><div class="c4-wrap"><div class="c4-section-heading"><p class="c4-label">The engagement</p><h2 id="working-together-title">${esc(c.heading)}</h2></div><p class="c4-prose">${esc(c.intro)}</p><div data-c4-engagement-media-slot data-media-status="approved">${ownerFigure('engagement')}</div><ol class="c4-engagement">${c.steps.map((s,i)=>`<li data-engagement-step="${s.id}" data-optional="${s.optional}"><span class="c4-stage-number">0${i+1}</span><div><h3>${esc(s.title)}</h3><p class="c4-engagement-price">${esc(s.price)}${s.optional&&s.id!=='system'?' · Optional':''}</p><p>${esc(s.description)}</p></div></li>`).join('')}</ol><p class="c4-prose c4-after">${esc(c.outro)}</p></div></section>`;
}
