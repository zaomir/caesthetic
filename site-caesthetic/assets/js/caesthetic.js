...
(function () {
  ...
  function initRequestModal() {
    ...
    var requestCopy = requestLocale === "ru" ? {
      close: "Закрыть форму", kicker: "Запрос", title: "Оставьте ваши данные", intro: "Мы ответим по email.",
      name: "Имя", email: "Email", send: "Отправить запрос", sending: "Отправляем…", sent: "Отправлено",
      success: "Запрос отправлен. Мы ответим по email и сообщим следующий коммерческий шаг.",
      questionSuccess: "Вопрос отправлен. Мы ответим по email.", failed: "Не удалось отправить запрос. Попробуйте ещё раз."
    } : {
    ...
    function metaFor(trigger) {
      ...
      if (trigger && trigger.hasAttribute("data-cae-sprint-inquiry")) return { kind:"sprint", intent:explicitIntent || "30_day_growth_sprint_2500", kicker:russian ? "30-дневный спринт роста · $2,500" : "30-Day Growth Sprint · $2,500", title:russian ? "Запросить спринт" : "Request your Sprint", intro:russian ? "Только имя и email. Мы подтвердим индивидуальный объём работ, письменный Order и приватные инструкции по оплате." : "Name and email only. We will confirm the practice-specific scope and send the written Order and private payment instructions.", submit:russian ? "Запросить спринт" : "Request Sprint" };
      if (trigger && trigger.hasAttribute("data-cae-check-inquiry")) return { kind:"check", intent:explicitIntent || "lead_to_revenue_check_500", kicker:russian ? "Проверка пути от обращения к выручке · $500" : "Lead-to-Revenue Check · $500", title:russian ? "Запросить проверку" : "Start the Lead-to-Revenue Check", intro:russian ? "Только имя и email. До оплаты мы подтвердим объём доказательств, письменный Order и приватные инструкции по оплате." : "Name and email only. We will confirm the evidence scope, written Order and private payment instructions before payment.", submit:russian ? "Запросить проверку за $500" : "Request the $500 Check" };
      if (trigger && trigger.hasAttribute("data-cae-growth-system-inquiry")) return { kind:"growth_system", intent:explicitIntent || "growth_system_inquiry", kicker:"Growth System", title:russian ? "Спросить о постоянном сопровождении" : "Ask about recurring ownership", intro:russian ? "Оставьте имя и email. Коммерческий объём и экономика определяются индивидуально." : "Leave your name and email. Commercial scope and economics stay client-specific.", submit:russian ? "Отправить запрос Growth System" : "Send Growth System request" };
      if (trigger && trigger.hasAttribute("data-cae-question")) return { kind:"question", intent:explicitIntent || "question", kicker:russian ? "Вопрос" : "Question", title:russian ? "Задать вопрос CAESTHETIC" : "Ask CAESTHETIC a question", intro:russian ? "Только имя и email. Мы ответим по email и уже знаем, с какой страницы вы обратились." : "Name and email only. We will reply by email and already know which page you asked from.", submit:russian ? "Отправить вопрос" : "Send question" };
