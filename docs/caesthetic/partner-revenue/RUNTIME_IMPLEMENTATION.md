...
В Twenty передаются B2B-атрибуты, остатки и финансовые агрегаты. Запись проверяется чтением. Outbox переживает недоступность CRM; alarm обновляет проекции, повтор через 5 минут, ручная сверка доступна ops. Ручная кнопка сверяет выбранную программу и проверяет scope. Пакетный API без `program_id` возвращает не более 10 программ и `next_cursor`; продолжение передаёт его как `after`. Это ограничивает три внешних запроса на программу (lookup/write/readback) и не теряет программы за пределами первой пачки. Редактирование проекции Twenty не меняет registry/ledger. Начисление Caesthetic не означает получение денег Caesthetic. B2B-переговоры и планы мероприятий ведутся в Twenty; клинические события остаются у клиента.
