...
New authoring emits `audit.profile_version="multi-location-growth-score/1.2.0"`. This is a presentation/envelope profile marker only: `schemaVersion=5` and `templateVersion=growth-score-report-template/5.2.0` remain immutable. Parents without `profile_version` and frozen profile-1.1 packages remain readable without silent migration. Profile 1.2 is fail-closed for every new field.

A `network_parent` adds the network registry, declared/reviewed coverage, shared and local public assets, per-location Journey Graph references, repeated-pattern evidence, location × Four Surfaces comparison and network scope/rollout fields for the exact Top 3. Profile 1.1 requires `focus_decision`, `executive_summary`, `propagation_candidates` and `publication_approval`; profile 1.2 retains them and adds `network.decision_intelligence`. A `focus_location` remains a complete ordinary schema-v5 report and adds only the package envelope and parent navigation.

`network.decision_intelligence` uses `artifact_version="multi-location-decision-intelligence/1.0.0"` and `source_policy="existing_growth_score_evidence_only"`. It contains exactly one `location_projection` for every reviewed location and none for unreviewed locations. Each projection embeds a complete approved `decisionViews` artifact validated by the existing schema-v5 engine, or an explicitly approved empty `not_assessed` artifact. Reused treatment/provider IDs must resolve consistently. The envelope and each embedded artifact keep all four automation flags false; any `network_score`, `treatment_score`, `provider_score`, `friction_score` or equivalent numeric decision score is invalid.

`scripts/caesthetic/multi-location-decision-view-model.mjs` is a pure presentation projection. It derives Treatment × Location, Provider × Location, up to five initially visible representative Trust Chains, categorical Patient Friction by location and treatment-specific promotion-hold groups. Missing treatments/providers remain `not_assessed`; absence never becomes a gap or permission to promote. Additional locations after the first four and additional chains after the first five remain in native disclosure controls and in the HTML.

`focus_decision.criteria` contains exactly `public_journey_risk`, `evidence_confidence`, `thirty_day_feasibility` and `network_learning_value`; each has an assessment and non-empty public evidence references, and `not_business_performance_ranking` is exactly `true`. Every selected gap additionally carries `execution_owner=hq|local|shared`, `accountable_role`, `public_baseline` and `day_30_public_check`. A propagation candidate names its reviewed source and target locations, surface, evidence references, standardization instruction and limitation. The publication approval must be `approved`, match the report reviewer name/timestamp, focus location and ordered Top 3, and assert `public_sources_only=true`.

The canonical renderer selects the network presentation profile only for `audit.format=multi_location` plus `audit.package_role=network_parent`. `scripts/caesthetic/multi-location-growth-score-view-model.mjs` orchestrates the pure client-presentation adapters: they may derive labels, focus-first ordering, affected-location summaries, categorical counts and progressive-disclosure groups, but they must not mutate evidence, scores, coverage, the binding constraint or the manager-approved Top 3. A focus child uses the ordinary location visual profile. Parent and child package validation must fail on identity, access group, route, focus location, ordered Top 3, binding constraint or Do Not Fund Yet mismatch. A profile-1.2 parent also fails when a reviewed location lacks a decision projection, an unreviewed location participates, evidence is not approved, IDs conflict, a numeric decision score appears, a hold lacks its engine-level human approval, or the network decision review does not match the report reviewer.

Within the same nine-section order, a network parent renders an Executive Network Decision Summary in the hero. `gap-map` derives risk counts from the location matrix, shows the focus criteria, renders that internal matrix once and follows with the shared/local/repeated Network Overview. `focus-gaps` renders one compact card per approved priority with owner, baseline and public Day-30 check. `sprint-fit` derives the 30-day sequence; `repair-paths` shows ownership and rollout gates; `gap-inventory` may show propagation candidates; `evidence-and-competitors` shows a compact competitor summary before its two full disclosures; `scores-and-methodology` states the public/internal evidence boundary; and `next-step` shows explicit CMO decisions before the single CTA. No derived risk count is persisted or scored. Additional locations after the first four remain in the document under disclosure. The focus child renders a parent breadcrumb and no second commercial CTA.
