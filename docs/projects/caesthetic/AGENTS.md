...
- Growth System uses one client-specific Growth Budget: its Fixed Management Fee is a visible line inside the budget, variable inputs use the remaining approved funds, and any contractually activated Performance Fee sits separately above the budget. No reusable recurring amount, rate or cap.
