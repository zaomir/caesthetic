...
Primary direct cold sender: `caesthetic.co`
...
## Audit / report / score assignments
...
- Acquisition / prospect qualification / signals / A-B-C-Research-Reject / multichannel activation / cold email/Instantly / 9-word/micro-email / Private Growth Preview / pre-Score boundary / Partner Growth Score / referrals / CITY CHECK sequencing / acquisition cockpit → `docs/ssot/CAESTHETIC_ACQUISITION_DIAGNOSTIC_GROWTH_CONTROL_SYSTEM.md` (DEC-845).
...
- Hooppy / posting job / agent handoff / entertainment videos / reels inbox / recurring social rotation / cycle restart / publication tracking → first read `docs/ssot/CAESTHETIC_HOOPPY_POSTING_OPERATOR.md`: agent handoff is final publication authority and requires no second approval or editorial/compliance gate; then read the defaults/evidence contract, `docs/projects/caesthetic/operations/ig-growth/11-ENTERTAINMENT_REELS_ROTATION_PLAN.md` §13, `docs/ssot/HOOPPY_API.md` and SIMON_OPS `CAE_Entertainment_Rotation`.
...
- `/beauty-salons/` plus localized routes — Beauty Salons vertical adapter
- `/about/` — legal entity and public operating identity

Sender rules are semantic, not inferred from an existing route: `caesthetic.co` is direct; `bototox.com`/`toxifillers.com` are conditional historical reactivation; all other owned identities remain separate. One account receives one active narrative and sender; global suppression applies.

The retired `/outreach/` verification page has no redirect authority and returns HTTP 404. Public verification-hub content must not be recreated under another route.
