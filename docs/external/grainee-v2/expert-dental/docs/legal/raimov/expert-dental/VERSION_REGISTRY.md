...
status: active-register / counsel-approved-baseline / tablet-release-1.0.0 / no-in-use-yet
...
Counsel approval and release numbering are not the same as `IN_USE`. Medical-template approval, licence/service/provider clearance, tablet QA and exact per-form activation remain independent gates.

## Обязательные поля версии

Каждая форма должна содержать на самом документе и в реестре:

- document ID из `DOCUMENT_REGISTER.md`;
- название;
- номер версии;
...
- язык;
- SHA256 утверждённого пустого файла/PDF;
- required signers;
- retention class;
- статус;
...
A version appears in this table only after exact tablet/PDF artifact generation, required medical and evidence clearance, visual/accessibility QA and owner go-live decision.
...
2. После `effective date` only the current `IN_USE` version is available to staff for new signing sessions.
3. Подписанный экземпляр навсегда связан с той версией, которая действовала в момент подписания.
...
7. CRM/DMS rejects an unknown, blocked, expired or non-effective version ID and stores the final signed-artifact hash.
