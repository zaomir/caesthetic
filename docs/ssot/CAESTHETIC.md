...
→ mid-report: show the optional Lead-to-Revenue Check after explaining the post-enquiry path
→ when an actionable external priority is verified: 30-Day Sprint — fix it and verify adoption
→ final decision: show the same Check again as a smaller optional first engagement after the Sprint offer
...
The sole operating sequence is `docs/ssot/CAESTHETIC_GROWTH_SCORE_PRODUCTION_SOP.md`, authoring route 3.0.0. Resolve the minimum business identity/location and reuse supplied context. AI autonomously records scope, public sources, competitors and uncertainties, researches, and creates the complete Russian audit before asking the manager to review its work. Research Alignment is an internal planning record, not an approval gate. Do not stop for checks of source lists, service scope, query phrases, evidence or candidate priorities. Missing public evidence is disclosed in the Russian audit; it does not justify inventing a score or withholding the entire document.

The first review of AI work is the complete Russian audit with the full Gap Inventory, candidate conclusions and repair paths. If fewer than three material problems are supported, say so inside that audit rather than filling mandatory slots with minor issues. A named manager then verifies the evidence, selects the final one Primary plus two Supporting Gaps when supported, and approves the exact Russian version. Those final approvals still control fact freeze, translation and publication. An internal Russian audit is not an approved or delivered client report. Prior recorded approvals remain valid history.
