...
version: 1.5.0
updated: 2026-09-05
explanation_copy_status: draft-for-discussion
scope: Connect4 concept; client-neutral explanation; full landing and reusable section proposal; approved visual contracts
...
Семантические решения в §2–7 закреплены для CAESTHETIC: четыре плоскости, согласованность, один ответственный, честные отзывы, отдельный Lead Intake, позднее объяснение рекламы, различение внедрения и эффекта. Клиника/пациент в этих операционных разделах — отраслевой контекст CAESTHETIC, не обязательный словарь общего объяснения. Запрос владельца от 2026-09-05 задаёт нейтральный словарь для новых общих описаний: бизнес, потенциальный клиент, обращение, клиент, следующий шаг (§10.2). Это не меняет клиническую терминологию, факты существующих кейсов, ICP или договоры.

Точные изображения отдельно утверждены в §9.1–9.3. Текст и композиции §10 — текущая единая рабочая редакция, **не утверждённый Copy Lock и не опубликованная страница**. Ранние варианты из §8 и прежнего §10 заменены для дальнейшего обсуждения этой редакцией; история сохранена в git. Запрос картинки не означает утверждения изображения.

Метод исследования, десять фраз, матрица 10 × 4 и статусы доказательств принадлежат `CAESTHETIC_4444_CONSISTENCY_STANDARD.md`. Цены, продуктовая лестница и границы исполнения принадлежат master и operating-model SSOT. Этот выпуск меняет только данный документ: не runtime, готовые отчёты, скоринг, договоры, изображения, инструкции других проектов или механизм синхронизации репозиториев.
...
Закреплённое начало: Hero → объяснение Connect4 / «Как мы работаем». Затем — кейсы с проверяемыми исходным состоянием, работой и результатом. Stop / Do Not Fund Yet всегда ниже объяснения четырёх плоскостей и Lead Intake; рекомендация текущей редакции — после кейсов, но точное место ещё не утверждено.
...
Для общей схемы системы: название Connect4; полный состав Search & Maps, Website, Social, Reviews & Reputation; общий объект в центре; самостоятельный Lead Intake ниже. Новые нейтральные подписи и композиции предложены в §10.8, но не изменяют текст внутри утверждённых PNG.
...
### 10.3 Ядро и короткое объяснение — EN/RU draft
...
Проверка открытых источников: 2026-09-05. Ниже отделены данные источников от нашей редакторской интерпретации. Заголовки болей — наши формулировки, не цитаты респондентов.

| Боль для сообщения | Что найдено | Граница вывода |
|---|---|---|
...
Рекомендуемая основная боль: **«Мы вкладываемся в маркетинг, но клиенту всё ещё трудно понять предложение и сделать следующий шаг».** Поддерживающие темы: перегрузка владельца, нет общего приоритета, работа начинается заново при смене подрядчика. Последние две не продавать как доказанные у всех — проверять на целевых интервью.
...
- Google Maps, правила отзывов, проверено 2026-09-05: https://support.google.com/contributionpolicy/answer/7400114?hl=en
...
### 10.6 Полная страница — proposed landing narrative

Предлагаемый маршрут: `/connect4/`. Это проект адреса, не созданный или опубликованный URL. Публичный заголовок и навигация — Connect4; не `/4444/` как второе имя. CAESTHETIC специализируется на med spas/beauty в разрешённых адаптерах, хотя базовая формулировка нейтральна.
...
### 10.7 Одна переносимая секция — текущий draft
...
Пользователь предоставит реальные изображения схем. В этой задаче новые картинки не создаются. Ниже — назначение мест, не свидетельство наличия новых утверждённых assets.
...
### 10.9 Как не потерять единое понимание между сессиями и публикациями

Смысловое правило для дальнейшего обсуждения: Connect4, 4444 и «Четверки» разрешаются в этот документ на актуальном main. Не выводить концепцию из числа четыре и не пересобирать определения из chat memory. В общем объяснении использовать §10; §2–7 остаются medspa-операционным адаптером. Переносимость словаря не даёт права менять цены/факты/диагнозы между нишами.

Для утверждения и реализации предлагается один набор блоков с постоянными идентификаторами: `definition`, `value`, `surfaces`, `mechanism`, `reviews`, `intake`, `ownership`, `handoff`, `proof`, `cta-context`. У каждого — одна версия EN и связанный русский смысл. Длинная страница раскрывает эти блоки, компактная секция использует разрешённое сокращение. Новый слоган в другом чате не заменяет определение автоматически.

В runtime после отдельного задания должен использоваться один reusable content module, а не вручную скопированные описания в каждом HTML/отчёте. Версия текста и версия картинки независимы, но совместимость проверяется. Документ — смысловая authority, машинный модуль — исполняемое представление, не второй канон. Пока такой module/guard не внедрён, нельзя заявлять автоматическое соответствие всех публикаций.

Предлагаемые проверки: точное имя Connect4; ровно четыре surface IDs и правильные подписи; никакой пятой поверхности; reviews остаются независимыми; Lead Intake отдельно и без диагноза из внешних данных; реклама только после объяснения; один ответственный; scope и передача; Shipped/Adopted/Impact отдельно; локали согласованы; client-specific facts изолированы; утверждённые PNG/hash не меняются; отсутствие свежих generic patient terms вне явного медицинского контекста; CTA соответствует месту. Эти проверки описаны, не реализованы этим коммитом.
...
### 10.10 Минимальная проверка сообщения и открытые решения

Следующее проверяемое действие — не строить новый продукт, а проверить понимание и узнаваемость боли на небольшом направленном пилоте: например, пять владельцев/управляющих и три маркетолога, включая сеть. Это предложенная выборка, не уже выполненное исследование и не статистическая оценка рынка.

До показа спросить о последнем конкретном эпизоде: что не работало, кто участвовал, как решали, за что уже платили. После просмотра попросить своими словами объяснить, что делает Connect4, чем оно отличается от ведения каналов, что останется у бизнеса и где заканчивается включённая работа. Не задавать наводящее «вам мешают несогласованные подрядчики?».

Предложенный критерий редакторской приёмки: как минимум 4 из 5 владельцев и 2 из 3 маркетологов верно называют связь четырёх плоскостей, внедрение/передачу и границу Lead Intake; никто не выводит гарантию новых клиентов или обязательную подписку. Это design target для пилота, не отраслевой benchmark. Если сообщение воспринимается как абстрактный консалтинг, исправлять примеры и deliverables, а не добавлять обещание выручки. Если целевые собеседники не узнают основную боль и не вспоминают реальных эпизодов, отказаться от этой pain-led формулировки, не объявлять проблему существующей вопреки данным.

Публикационная проверка после реализации: одинаковая версия ядра на подробной странице, в Кейcах и разрешённых блоках новых отчётов; понятность desktop/mobile; корректные реальные картинки; существующий критический CTA-путь. Это проверка внедрения, не доказанный коммерческий эффект.

Открытые решения: финальный Copy Lock; конкретные user-supplied картинки для каждого места и их нейтральные редакции; утверждение публичного маршрута `/connect4/`; точное положение Stop после кейсов; фактическая ответственность и scope в договоре. Неизменные решения — публичное имя, четыре плоскости, отдельный Intake, честные отзывы, один владелец логики, необязательное продолжение.

## 11. История восстановления и граница результата этой редакции
...
История показывает: рабочая EN/RU-редакция v1.2 была сохранена в `5667f4b6dcd83efcb07e5cd498992d39b66f95d1`; мобильный маршрут добавлен PR #1494 / `d79d62194d6c5387f47829142592d8d01fb4d33a`; мобильный Stop добавлен PR #1495 / `e52475bc616fdce7cafc6967f0fc29b95ec507cf` (v1.4). Затем sync-коммит `7eba16b7d5eb18c2e736d23d7f63dfeadcaf2b64` от 2026-09-05 10:47:57Z заменил v1.4 на v1.1 и убрал соответствующие записи из AGENTS/PROJECT_STATUS. Это наблюдаемый diff; глубинная причина работы sync-алгоритма этой задачей не исследована и не исправлена.

В v1.5 восстановлены записи утверждённых мобильных контрактов §9.2–9.3 из v1.4; §9.1 сохранён. Бинарные assets не менялись. Предыдущее draft-объяснение переработано в текущую нейтральную редакцию §10, не объявлено утверждённым. Соседние обсуждения использованы в объёме доступного проектного контекста и сохранённой истории репозитория; исчерпывающее чтение всех соседних чатов не подтверждено.

Изменён только этот SSOT. Полная защита от повторного downgrade, восстановление согласованных ссылок AGENTS/PROJECT_STATUS, общий runtime-модуль и миграция публикаций остаются отдельной задачей внедрения. Не считать сохранение этого документа доказательством, что Connect4 уже одинаково опубликован везде.
