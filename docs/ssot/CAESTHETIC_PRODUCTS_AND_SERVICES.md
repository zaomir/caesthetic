...
| 30-Day Growth Sprint | Внедрение согласованного priority constraint и необходимых зависимостей за 30 дней; законченные исправления плюс реально запущенная работа с более длинным горизонтом. | price confirmed after scope review; мастер §5. Top 3 — приоритеты, а не обязанность продать ровно три услуги. |
...
| Sprint Extension | Дополнительная конечная реализация, если её обосновывает Day-30 evidence, доступ, vendor dependency или объективный срок. | price confirmed after scope review за дополнительные 30 дней; только после Day 30, не обязательный upfront product. |
...
сопровождение по отдельно согласованному scope. Это условия конкретного предложения, а не
...
основанием для продажи Sprint по цене, подтверждённой после уточнения объёма. Сначала оценить проблему для пациента,
...
2. Ценность исполнения CAESTHETIC: какая связанная работа требует нашей
   ...
   самому клиенту; почему CAESTHETIC предлагает именно этот scope и цену для этого бизнеса.
...
продажу Sprint только за эту правку.
...
Existing First Sprint interventions now have paired public EN explanations: `/connect4/what-we-build/` for the practical four-surface build (catalog → social UGC → Maps + honest review loop → local platforms), `/connect4/location-alignment/` for correcting verified cross-surface inconsistencies and `/connect4/review-system/` for adapting our own review/feedback collection system. Generators: `scripts/caesthetic/build-connect4-services.mjs` (service pages) and handcrafted `/connect4/what-we-build/`. Reports link to the relevant explanation from evidence-backed repair plans; mention of feedback collection does not diagnose an unseen internal workflow. Public explanations disclose outcomes, ownership and boundaries without internal routing mechanics.
