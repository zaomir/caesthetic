...
test("Russian Spoken report presents the approved owner-first sequence without empty or duplicate blocks", () => {
  ...
  assert.match(storedHtml, /<h2 class="cae-owner-research__title">Изученные ссылки<\/h2>/);
  ...
  assert.equal((researchHtml.match(/<li><a /g) || []).length, 3);
  assert.equal((researchHtml.match(/target="_blank"/g) || []).length, 3);
  assert.match(researchHtml, />Сайт<\/a>/);
  assert.match(researchHtml, />Карты <span data-brand>Google<\/span><\/a>/);
  assert.match(researchHtml, />Социальные сети<\/a>/);
  ...
  assert.match(reportCss, /\.cae-owner-research__links\s*\{[^}]*display:\s*flex;[^}]*flex-wrap:\s*wrap;/s);
  assert.match(reportCss, /\.cae-owner-research__links a\s*\{[^}]*min-height:\s*44px;[^}]*border:\s*0;[^}]*background:\s*transparent;/s);
