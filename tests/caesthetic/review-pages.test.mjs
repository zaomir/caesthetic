...
test('real review pages open directly and retain honest draft metadata',()=>{
  ...
  for(const p of registry.pages.filter(p=>p.stage==='manager_review')){
    assert.equal(p.accessMode,'none');
    assert.ok(!manifest.scoreProtectedPaths.some(x=>p.route.startsWith(x.prefix)));
